        ...
        <div className="relative mx-auto w-full max-w-[1500px] px-5 sm:px-8 lg:px-12">
          ...
          <header className="mt-8 border-b border-line pb-8">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-1">
              {project.kind && <span className="text-base text-muted sm:text-lg">{pick(project.kind)}</span>}
              {project.kind && project.client && <span className="h-1 w-1 bg-line-strong" aria-hidden />}
              {project.client && <span className="label-a">{project.client}</span>}
            </div>
